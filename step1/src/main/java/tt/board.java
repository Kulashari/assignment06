...
public class board { //board class that represents the board
    ...
    public void display() { //displays the baord pattern, with filled or non filled squares
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) { //goes through the 2d char array in a nested for loop
                System.out.print(" " + grid[i][j] + " "); //places the player symbol in the according row and column
                if (j < 3 - 1) { //if not the last column it prints vertical separator between cells
                    System.out.print("|");
                }
            }
            System.out.println(); //goes to next row
            if (i < 3 - 1) { //if not the last row it prints horizontal separator between rows
                System.out.println("-----------");
            }
        }
